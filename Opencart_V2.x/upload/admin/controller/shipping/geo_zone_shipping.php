<?php
class ControllerShippingGeoZoneShipping extends Controller {
	...
	public function index(){
		...
		$data = $this->load->language('shipping/geo_zone_shipping');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('shipping/geo_zone_shipping', 'token=' . $this->session->data['token'] . '&type=shipping', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/geo_zone_shipping', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('shipping/geo_zone_shipping', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension', 'token=' . $this->session->data['token'] . '&type=shipping', true);
		...
		$this->response->setOutput($this->load->view('shipping/geo_zone_shipping.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'shipping/geo_zone_shipping')) {
